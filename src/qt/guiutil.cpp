...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("goatcheese"))
        return false;
...
{
    // Convert goatcheese:// to goatcheese:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("goatcheese://"))
    {
        uri.replace(0, 12, "goatcheese:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GoatCheese.lnk";
}
...
{
    return GetAutostartDir() / "goatcheese.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GoatCheese-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  goatcheese-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GoatCheese-Qt"));
